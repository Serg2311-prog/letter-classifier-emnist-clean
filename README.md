...
- CNN показывает наилучший результат (94.21%), так как учитывает пространственную структуру изображений и извлекает локальные признаки (контуры, формы)
- MLP достигают ~91% точности, но уступают CNN из-за отсутствия обработки пространственных зависимостей
- Dropout оказывает незначительное влияние, что указывает на слабое переобучение моделей

## Вывод

Сверточная нейронная сеть (CNN) является наиболее эффективной архитектурой для данной задачи, обеспечивая прирост точности по сравнению с MLP при меньшем числе параметров.
