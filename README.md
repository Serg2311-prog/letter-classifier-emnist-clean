...
- Модель CNN показала наилучший результат (94.21%), так как способна учитывать пространственную структуру изображений и извлекать локальные признаки (контуры, формы)
- Модели MLP достигают точности около 91%, но уступают CNN, поскольку не учитывают пространственные зависимости между пикселями
- Использование Dropout оказало незначительное влияние на качество MLP, что говорит об отсутствии сильного переобучения

## Вывод

Сверточная нейронная сеть (CNN) является наиболее эффективной архитектурой для задачи распознавания рукописных букв EMNIST в рамках данного проекта.
